...
# Show the full dataset (scrollable)
...
# ----------------------------------------------------------------------
# Create tabs: Overall and Client Specific
...
with client_tab:
    ...
    if "Client" not in data.columns:
        ...
        # Search bar that filters the dropdown list
        search_query = st.text_input("Search client name")
        filtered_client_list = [
            c for c in unique_clients if search_query.lower() in c.lower()
        ] if search_query else unique_clients

        if not filtered_client_list:
            st.info("No clients match your search.")
        # Dropdown that always uses the filtered list
        selected_client = st.selectbox(
            "Select a client", options=filtered_client_list
        )
        ...
        if client_data.empty:
            ...
            if "ResourceType" in client_data.columns and "CostUSD" in client_data.columns:
                resource_costs_client = (
                    client_data.groupby("ResourceType")["CostUSD"].sum().reset_index()
                )
            ...
            if "Region" in client_data.columns and "CostUSD" in client_data.columns:
                region_costs_client = (
                    client_data.groupby("Region")["CostUSD"].sum().reset_index()
                )
                fig_region_client = px.bar(
                    region_costs_client,
                    x="Region",
                    y="CostUSD",
                    title="Client Cost by Region",
                )
            ...
            if "CostCenter" in client_data.columns and "CostUSD" in client_data.columns:
                cc_costs_client = (
                    client_data.groupby("CostCenter")["CostUSD"].sum().reset_index()
                )
            ...
            if "CostUSD" in client_data.columns:
                ...
                top5 = (
                    client_data.sort_values(by="CostUSD", ascending=False)
                    .head(5)
                    .copy()
                )
            ...
            if "UsageHours" in client_data.columns and "CostUSD" in client_data.columns:
                client_data = client_data.assign(
                    CostPerHour=client_data["CostUSD"] / client_data["UsageHours"]
                )
                threshold = client_data["UsageHours"].mean() * 0.2
                underutilized = (
                    client_data[
                        (client_data["UsageHours"] < threshold)
                        & (client_data["CostUSD"] > 0)
                    ]
                    .sort_values(by="CostPerHour", ascending=False)
                )
                if not underutilized.empty:
                    st.subheader(
                        "Underutilized resources (high cost per hour)"
                    )
                    st.dataframe(
                        underutilized[
                            [
